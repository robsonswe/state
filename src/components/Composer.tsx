...
import { Smile, Frown, Meh, Send, Link as LinkIcon } from 'lucide-react';
import { cn } from '../lib/utils';
import { differenceInMinutes } from 'date-fns';

const MOODS: { value: MoodRating; icon: React.ElementType; label: string; color: string }[] = [
  { value: 1, icon: Frown, label: 'Awful', color: 'text-accent-rust bg-accent-rust-light' },
  ...
  { value: 5, icon: Smile, label: 'Great', color: 'text-accent-sky bg-accent-sky-light' },
];
...
export function Composer() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-paper/95 backdrop-blur-md border-t border-paper-dark p-6 pb-safe z-50">
      <div className="max-w-2xl mx-auto flex flex-col gap-4">
        ...
        <textarea
          ...
          className="w-full resize-none outline-none text-xl font-serif leading-relaxed bg-transparent placeholder:text-ink-faint text-ink max-h-[200px] overflow-y-auto"
          rows={1}
        />
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <button
            ...
            className="p-3 bg-ink text-paper rounded-full disabled:opacity-30 transition-opacity"
          >
